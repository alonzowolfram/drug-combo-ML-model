...
with open(db_xml) as xml_file:
    in_pathways_tag = False
    for event, drug in et.iterparse(xml_file, events=('start', 'end')):
        if drug.tag == et.QName(db_uri, 'pathways'):
            in_pathways_tag = event == 'start'
        if drug.tag == et.QName(db_uri, 'drug') and not in_pathways_tag:
            # Get all the drugs, i.e. all the elements named "drug" that are NOT CHILDREN of a "pathways" tag.
                ...
                try:
                    ...
                    print("Was unable to get a name for the current drug. Moving on to the next one.")
                    continue
                
                print("Checking targets for drug " + drug_name + ".")
